...
static inline uint8_t* RDP_TranslateAddr(uint32_t addr) {
    ...
    if (seg == 1 && s_rdp.segmentBase[1] == 0x80000000u && s_current_cmd) { static int s_seg1_entered = 0; if (s_seg1_entered++ < 5) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1ENTER dl=0x%lX segbase=0x%lX off=0x%X addr=0x%08X", (unsigned long)(uintptr_t)s_current_cmd, (unsigned long)s_rdp.segmentBase[1], off, addr); uintptr_t dl = (uintptr_t)s_current_cmd; for (uintptr_t p = (dl - 0x30) & ~0xFULL; p > dl - 0x40000; p -= 0x10) { if (!bka_is_readable((void*)p) || !bka_is_readable((void*)(p+32))) continue; uint8_t* a = (uint8_t*)p; uint8_t* b = (uint8_t*)(p+16); uint8_t* c = (uint8_t*)(p+32); int ok = 1; uint8_t* qs[3] = {a,b,c}; for (int i = 0; i < 3; i++) { uint8_t* q = qs[i]; int sig = (q[12] == 0x80 && q[13] == 0x7F && q[14] == 0x00 && q[15] == 0x00) || (q[12] == 0x7F && q[13] == 0x80 && q[14] == 0x00 && q[15] == 0x00); if (!sig) { ok = 0; break; } int16_t z = (int16_t)((q[4] << 8) | q[5]); if (z != 0) { ok = 0; break; } } if (!ok) continue; static int s_sig = 0; if (s_sig++ < 20) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1SIG dl=0x%lX vtxbuf=0x%lX off=0x%X", (unsigned long)dl, (unsigned long)p, off); return (uint8_t*)(p + off); } }
