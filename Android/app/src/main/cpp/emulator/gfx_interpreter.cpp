...
static inline uint8_t* RDP_TranslateAddr(uint32_t addr) {
    ...
    if (seg == 1 && s_rdp.segmentBase[1] == 0x80000000u && s_dl_base) { uintptr_t vtx = s_dl_base - 0xC80; static int s_fix = 0; if (s_fix++ < 5) { uint8_t* pp = (uint8_t*)vtx; int rd = bka_is_readable((void*)vtx); __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1FIX dlbase=0x%lX vtx=0x%lX off=0x%X rd=%d bytes=%02X%02X %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X", (unsigned long)s_dl_base, (unsigned long)vtx, off, rd, rd?pp[0]:0,rd?pp[1]:0,rd?pp[2]:0,rd?pp[3]:0,rd?pp[4]:0,rd?pp[5]:0,rd?pp[6]:0,rd?pp[7]:0,rd?pp[8]:0,rd?pp[9]:0,rd?pp[10]:0,rd?pp[11]:0,rd?pp[12]:0,rd?pp[13]:0,rd?pp[14]:0,rd?pp[15]:0); } if (bka_is_readable((void*)vtx)) return (uint8_t*)(vtx + off); }
