...
static inline uint8_t* RDP_TranslateAddr(uint32_t addr) {
    ...
    if (seg == 1 && s_rdp.segmentBase[1] == 0x80000000u && s_current_cmd) { uintptr_t c = (uintptr_t)s_current_cmd; uintptr_t best = 0; for (uintptr_t v = c - 0x2000; v + 0x40 <= c + 0x200; v += 0x10) { if (!bka_is_readable((void*)v)) continue; uint8_t* b = (uint8_t*)v; int good = 0; for (int i = 0; i < 4; i++) { uint8_t* q = b + i * 16; if (!bka_is_readable((void*)(q + 16))) break; int16_t x = (int16_t)((q[0] << 8) | q[1]); int16_t y = (int16_t)((q[2] << 8) | q[3]); int16_t z = (int16_t)((q[4] << 8) | q[5]); int cs = (q[12] == 0x80 && q[13] == 0x7F) || (q[12] == 0x7F && q[13] == 0x80); if (z == 0 && x > -2048 && x < 2048 && y > -2048 && y < 2048 && cs) good++; } if (good >= 3) { best = v; break; } } if (best) { static int s_hit = 0; if (s_hit++ < 4) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1VTX vtx=0x%lX delta=-0x%lX cur=0x%lX", (unsigned long)best, (unsigned long)(c - best), (unsigned long)c); return (uint8_t*)(best + off); } }
