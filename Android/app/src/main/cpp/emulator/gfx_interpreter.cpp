...
static inline uint8_t* RDP_TranslateAddr(uint32_t addr) {
    ...
    if (seg == 1 && s_rdp.segmentBase[1] == 0x80000000u && s_dl_base) { uintptr_t dl = s_dl_base; uintptr_t vtx = dl - 0xC80; static int s_fix = 0; if (bka_is_readable((void*)vtx)) { uint8_t* p = (uint8_t*)vtx; int hits = 0; for (int i = 0; i < 4; i++) { uint8_t* q = p + i*16; int m = (q[12]==0x80 && q[13]==0x7F && q[14]==0 && q[15]==0) || (q[12]==0x7F && q[13]==0x80 && q[14]==0 && q[15]==0); if (m && q[4]==0 && q[5]==0) hits++; } if (s_fix++ < 5) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1FIX dlbase=0x%lX vtx=0x%lX off=0x%X hits=%d/4", (unsigned long)dl, (unsigned long)vtx, off, hits); if (hits >= 2) return (uint8_t*)(vtx + off); } for (uintptr_t q = dl - 0x4000; q < dl; q += 0x10) { if (!bka_is_readable((void*)q)) continue; uint8_t* b = (uint8_t*)q; int m = (b[12]==0x80 && b[13]==0x7F && b[14]==0 && b[15]==0); if (!m) continue; if (b[4] != 0 || b[5] != 0) continue; static int s_scan = 0; if (s_scan++ < 5) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1SCAN dlbase=0x%lX vtxfound=0x%lX delta=-0x%lX off=0x%X", (unsigned long)dl, (unsigned long)q, (unsigned long)(dl - q), off); return (uint8_t*)(q + off); } static int s_miss = 0; if (s_miss++ < 3) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1MISS dlbase=0x%lX — no vtx in +-0x4000", (unsigned long)dl); }
