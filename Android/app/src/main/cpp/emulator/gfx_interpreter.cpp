...
static inline uint8_t* RDP_TranslateAddr(uint32_t addr) {
    ...
    if (seg == 1 && s_rdp.segmentBase[1] == 0x80000000u && s_current_cmd) { uintptr_t c = (uintptr_t)s_current_cmd; uintptr_t cands[6] = { c - 0xC80, c - 0xC20, c - 0xB00, c - 0x600, c - 0x400, c - 0x200 }; for (int k = 0; k < 6; k++) { uintptr_t v = cands[k]; if (!bka_is_readable((void*)v)) continue; uint8_t* b = (uint8_t*)v; int hits = 0; for (int i = 0; i < 4; i++) { uint8_t* q = b + i*16; if (!bka_is_readable((void*)(q+16))) break; int16_t z = (int16_t)((q[4] << 8) | q[5]); int cn = (q[12] == 0x80 && q[13] == 0x7F) || (q[12] == 0x7F && q[13] == 0x80); if (z == 0 && cn) hits++; } if (hits >= 2) { static int s_v = 0; if (s_v++ < 6) __android_log_print(ANDROID_LOG_ERROR, "BKA_GFX", "SEG1VTX2 k=%d cand=0x%lX hits=%d/4 cur=0x%lX", k, (unsigned long)v, hits, (unsigned long)c); return (uint8_t*)(v + off); } } }
