...
void ResourceMgr_HandleDma(void* dramAddr, uint32_t devAddr, uint32_t size) {
    ...
    extern int g_diag_assetId; extern void* g_diag_cfile; extern int g_diag_csize; if (g_diag_assetId) __android_log_print(ANDROID_LOG_INFO, "BKA_PI", "HandleDma: last asset=%d cfile=%p csize=%d", g_diag_assetId, g_diag_cfile, g_diag_csize);
    ...
    // MTE-safe read: use pread syscall to minimize libc stack usage
    int fd = fileno(f);
    if (size > 128 * 1024 * 1024) { LOGE("ResourceMgr_HandleDma: size too large: %u", size); fclose(f); memset(dramAddr, 0, 16*1024*1024); return; }
    ...
    if (tempBuf) {
        ...
        if ((romOffset & 0xFFFFF000) == 0x5000 || romOffset == 0x5E90 || romOffset == 0x5E94) {
            __android_log_print(ANDROID_LOG_INFO, "BKA-PI",
                ...
                tempBuf[8], tempBuf[9], tempBuf[10], tempBuf[11],
                tempBuf[12], tempBuf[13], tempBuf[14], tempBuf[15]);
        }
        if (bytesRead > 0) {
            memcpy(dramAddr, tempBuf, (size_t)bytesRead);
            if ((size_t)bytesRead < size)
                memset((uint8_t*)dramAddr + bytesRead, 0, size - (size_t)bytesRead);
        } else {
            memset(dramAddr, 0, size);
        }
        free(tempBuf);
