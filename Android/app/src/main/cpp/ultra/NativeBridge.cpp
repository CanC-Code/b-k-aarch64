...
Java_com_bkawrapper_NativeBridge_nativeInit(JNIEnv* env, jclass clazz, jobject activity) {
    ...
    [span_1](start_span)// Create GlobalRef so the activity reference doesn't expire[span_1](end_span)
    g_mainActivityObj = env->NewGlobalRef(activity);
    
    jclass activityClass = env->GetObjectClass(g_mainActivityObj);
    [span_2](start_span)// Find updateOtrProgress(int, String)[span_2](end_span)
...
Java_com_bkawrapper_NativeBridge_runOtrGeneration(JNIEnv* env, jclass clazz, 
                                                ...
                                                jstring outputDir) {
    [span_3](start_span)// Duplicate the FD so background threads can keep reading if Java closes its reference[span_3](end_span)
    int nativeFd = dup(romFd); 
    
    const char* outDir = env->GetStringUTFChars(outputDir, nullptr);
    AAssetManager* mgr = AAssetManager_fromJava(env, assetManager);
    
    [span_4](start_span)// Load the conversion manifest[span_4](end_span)
...
[span_5](start_span)// Stubs for game loop[span_5](end_span)
