...
// Capture the JVM pointer when the library loads
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    otr_builder_set_jvm(vm); // Send JVM to the builder logic
...
Java_com_bkawrapper_NativeBridge_nativeInit(JNIEnv* env, jclass clazz, jobject activity) {
    ...
    // Create a GlobalRef so the background thread can use the activity safely
...
Java_com_bkawrapper_NativeBridge_runOtrGeneration(JNIEnv* env, jclass clazz, 
                                                ...
                                                jstring outputDir) {
    // Duplicate the FD to prevent it from closing if the Service finishes early
    ...
    if (asset) {
        ...
        // This internal call now handles the background thread attachment
