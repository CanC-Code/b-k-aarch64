...
static void sha1_update(SHA1Ctx& ctx, const uint8_t* data, size_t len) {
    ...
    if(idx){ size_t fill=64-idx; if(len>=fill){ memcpy(ctx.buffer+idx,data,fill); sha1_transform(ctx.state,ctx.buffer); i+=fill; idx=0; } else { memcpy(ctx.buffer+idx,data,len); return; } }
...
static std::string sha1_hex(const uint8_t* data, size_t len){
    ...
    if((ctx.count&63)>55){ memset(ctx.buffer+(ctx.count&63)+1,0,63-(ctx.count&63)); sha1_transform(ctx.state,ctx.buffer); memset(ctx.buffer,0,56);}
...
std::vector<uint8_t> loadYAMLAsset(AAssetManager* mgr, const char* path){
    ...
    if(!asset){ LOGE("Failed to open asset: %s",path); return buf; }
                    ...
                    std::vector<uint8_t>& outOTR)
{
    ...
    std::vector<uint8_t> yamlBuf=loadYAMLAsset(mgr,yamlFile.c_str());
    if(yamlBuf.empty()) return false;

    // Delegate to legacy BKOTR loader for deterministic build
...
{
    outOTR.clear();
    if(!romData||romSize==0){ LOGE("Invalid ROM"); return false; }

    std::string sha=sha1_hex(romData,romSize);
    LOGI("ROM SHA1: %s",sha.c_str());

    static const std::unordered_map<std::string,std::string> romToBin={
        {"1fb13cad402518d3ae9a8dc4b52c5c54b2a4adc7","us_v10.bin"}
    };

    auto it=romToBin.find(sha);
    if(it==romToBin.end()){ LOGE("Unsupported ROM"); return false; }

    // Load BIN from assets
    std::ifstream f("/android_asset/otr_bins/"+it->second,std::ios::binary);
    if(!f){ LOGE("Failed to open BIN: %s",it->second.c_str()); return false; }
    f.seekg(0,std::ios::end);
    size_t size=static_cast<size_t>(f.tellg());
    f.seekg(0,std::ios::beg);
    outOTR.resize(size);
    f.read(reinterpret_cast<char*>(outOTR.data()),size);
    return true;
}
