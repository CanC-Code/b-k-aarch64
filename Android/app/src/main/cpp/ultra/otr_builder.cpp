...
void process_asset(int romFd, AssetEntry asset, const char* outDirPath) {
    ...
    uint8_t* decomp = decompress_rare_asset(comp.data(), asset.compSize, &actualSize);
    
    if (decomp) {
        int out = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0666);
        if (out != -1) {
            write(out, decomp, actualSize);
            close(out);
        }
        free(decomp);
    }
...
void run_native_otr_generation_with_callback(JNIEnv* env, jobject activity, jmethodID progressMid,
    ...
    if (header->magic != 0x424B414D) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "CRITICAL: Manifest Magic Mismatch!");
        return;
    }
    ...
    for (uint32_t i = 0; i < header->entryCount; i += 4) {
        ...
        // Safe Thread Attachment for UI Progress
        JNIEnv* myEnv;
