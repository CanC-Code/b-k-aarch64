...
def parse_splat_yaml(yaml_path):
    ...
    for seg in segments:
        if isinstance(seg, dict):
            ...
            # Look for subsegments which contain the actual files/offsets
            ...
            for sub in subsegments:
                if isinstance(sub, list) and len(sub) >= 3:
                    ...
                    file_type = sub[1]
                    file_name = sub[2]
                    # We store: offset, name (max 32 chars), type (max 8 chars)
...
def write_binary_manifest(entries, output_path):
    ...
    with open(output_path, 'wb') as f:
        ...
        for entry in entries:
            # Format: Offset(I), Name(32s), Type(8s)
