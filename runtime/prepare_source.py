...
class SourceHarmonizer:
    ...
    def enforce_header_guards(self, path, filename):
        """Physically adds #ifndef guards to every header to stop recursive inclusion errors."""
        ...
        # Create a unique guard name based on the filename
    ...
    def harmonize_file(self, path, filename):
        ...
        # [span_2](start_span)Exclude core headers from injection entirely to prevent duplication[span_2](end_span)
        is_core_header = any(x in filename for x in ["ultra64.h", "gbi.h", "mbi.h", "sptask.h", "enums.h", "ucode.h", "sp.h"])
        ...
        if needed_defs or prototypes:
            injection = "\n// --- AUTOMATED HARMONIZER v3.2 (AGGRESSIVE) ---\n" + "\n".join(needed_defs + prototypes) + "\n"
...
def prepare_source():
    print("--- Starting Automated Source Harmonization v3.2 ---")
    ...
    # 1. Enforce guards on all fresh headers FIRST
