...
// 2. Hardware and Audio Types (Defined EARLY to prevent circular include crashes)
...
def deploy_header_exorcist():
    root = Path.cwd().resolve()
    decomp = root / "decomp-files"
    include_dir = decomp / "include"
    
    print("--- [v144.0] DEPLOYING HEADER EXORCIST ---")
    
    # Write Master Bridge
    (include_dir / "n64_types.h").write_text(BRIDGE_CONTENT)

    # 1. DELETE local standard library headers to stop shadowing!
    std_headers = ["string.h", "math.h", "stdarg.h", "time.h", "basic_types.h"]
    for sh in std_headers:
        p = include_dir / sh
        if p.exists():
            print(f"Exorcising shadowed standard header: {sh}")
            p.unlink()

    # 2. Block N64 SDK headers safely
    ...
    print("--- Exorcism Complete. Run Ninja. ---")

if __name__ == "__main__":
    deploy_header_exorcist()
