...
class SourceHarmonizerV8_4:
    ...
    def index_all_symbols(self):
        print("  [>] Pass 2: Linkage Mapping & Deduplication...")
    ...
    def promote_and_clean(self):
        print("  [>] Pass 3: Globalizing Symbols & Alignment Stripping...")
        for root, _, files in os.walk(self.src_target):
            for f in files:
                if f.endswith('.c'):
                    ...
                    # Force all statics to global to prevent 'undefined reference' during large-model linking
                    content = re.sub(r'^static\s+', '', content, flags=re.MULTILINE)
                    # Strip N64 attributes that cause alignment faults on ARM64
    ...
    def generate_final_header(self):
        print("  [>] Pass 4: Generating Large-Model Linkage Header...")
        ...
        with open(header_path, 'w') as f:
            ...
            for sym, dtype in self.global_symbols.items():
                if sym.startswith(('D_', 'g', 'bgs', 'B_')):
                    ...
                    # v8.4: Using 'weak' + 'extern' ensures the large model deduplicates addresses correctly
    ...
    def patch_cmake(self):
        print("  [>] Finalizing CMake for v8.4 Large Model...")
        if os.path.exists(self.cmake_file):
            with open(self.cmake_file, 'r') as f: content = f.read()
            content = re.sub(r'# --- Harmonizer v[0-9]\.[0-9].*?# ---+', '', content, flags=re.DOTALL)
            # v8.4 Fixes: 
            # 1. -mcmodel=large: Solves ADR_PREL_PG_HI21 relocation errors
            # 2. -fvisibility=hidden + -Wl,--export-dynamic: Optimizes the symbol table
            injection = (
                "\n# --- Harmonizer v8.4 Gold Master ---\n"
                ...
                "set(CMAKE_EXE_LINKER_FLAGS \"${CMAKE_EXE_LINKER_FLAGS} -Wl,--allow-multiple-definition -Wl,--export-dynamic\")\n"
    ...
    def run(self):
        print("--- Harmonizer v8.4: Final Gold Master ---")
        self.parse_typedefs()
        self.sync_files()
...
if __name__ == "__main__":
    h = SourceHarmonizerV8_4("Android/app/src/main/cpp", "decomp-files")
    h.run()
