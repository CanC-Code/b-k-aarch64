...
class SourceHarmonizerV7_9:
    ...
    def index_all_symbols(self):
        print("  [>] Pass 2: Global Linkage Indexing...")
        blacklist = {'main', 'memcpy', 'memset', 'memmove', 'sprintf', 'sqrt', 'sin', 'cos'}
        # New pattern: Specifically looks for static functions to promote them
    ...
    def promote_and_inject(self):
        """
        New for v7.9: Strips 'static' from source files to allow global visibility
        and injects the master header.
        """
        print("  [>] Pass 3: Static-to-Extern Promotion & Injection...")
        for root, _, files in os.walk(self.src_target):
            for f in files:
                if f.endswith('.c'):
                    ...
                    # Convert 'static void func()' to 'void func()' so other files can see it
    ...
    def generate_final_header(self):
        ...
        with open(header_path, 'w') as f:
            ...
            for name, sig in self.func_signatures.items():
                f.write(f"__attribute__((weak)) {sig};\n")
            ...
            for sym, dtype in self.global_symbols.items():
                if sym.startswith(('D_', 'g', 'bgs')):
                    ...
                    # Use array bracket suffix for data symbols to satisfy Linker size requirements
    ...
    def run(self):
        print("--- Harmonizer v7.9: Final Linker Visibility ---")
        self.parse_typedefs()
        self.sync_files()
        self.index_all_symbols()
        self.generate_final_header()
        self.promote_and_inject()
        self.patch_cmake()
        print("--- v7.9 Complete ---")

if __name__ == "__main__":
    h = SourceHarmonizerV7_9("Android/app/src/main/cpp", "decomp-files")
    h.run()
