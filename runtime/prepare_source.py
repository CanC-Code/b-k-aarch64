...
// FIXED: Explicitly named struct ALWaveTable_s
...
// FIXED: Added ALSynth auxBus member
typedef struct {
...
// --- Hardware ADPCM & Decoding Macros ---
...
def deploy_hardware_decoder():
    root = Path.cwd().resolve()
    decomp = root / "decomp-files"
    include_dir = decomp / "include"
    
    print("--- [v117.0] DEPLOYING HARDWARE DECODER ---")
    
    bridge_path = include_dir / "n64_types.h"
    bridge_path.write_text(BRIDGE_CONTENT)

    # Nuke R4300.h which overrides K0_TO_PHYS
    toxic = [
    ...
    for h in toxic:
        p = include_dir / h
        if p.exists():
            p.write_text("/* Terminated by Hardware Decoder */\n")
    ...
    for path in decomp.rglob("*.[ch]"):
        ...
        try:
            ...
            # Dynamic Terminator for all conflicting types
    ...
    print("--- Hardware Decoder Deployed. Execute Build Sequence. ---")

if __name__ == "__main__":
    deploy_hardware_decoder()
