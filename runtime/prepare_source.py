...
class SourceHarmonizerV53:
    ...
    def ensure_environment(self):
        """Pass 0: Ensures directory integrity and fresh file synchronization."""
    ...
    def precheck_type_integrity(self):
        """NEW PRECHECK: Scans headers to prevent duplicate typedefs (Fixes errors 206, 218)."""
        print("[>] PreCheck: Mapping Existing Header Types...")
        # Matches 'typedef struct Name Name;' or '} Name;'
                ...
                if file.endswith('.h'):
                    with open(os.path.join(root, file), 'r', errors='ignore') as f:
                        ...
                        for match in typedef_pattern.findall(content):
                            ...
                            self.predefined_types.add(type_name)
    ...
    def map_and_promote(self):
        ...
        for root, _, files in os.walk(self.src_target):
            for f in files:
                ...
                # Only discover types NOT already found in the PreCheck
                for t in type_regex.findall(content):
                    if t not in self.predefined_types and t not in self.reserved_identifiers:
    ...
    def generate_header(self):
        """Pass 3: Finalizes the header using opaque pointers (Fixes error 10)."""
        ...
        with open(header_path, 'w') as f:
            ...
            # Use Opaque Pointers for discovered types to avoid 'anonymous struct' errors
            for t in sorted(self.discovered_types):
                f.write(f"#ifndef {t}_DEFINED\n  struct {t};\n  typedef struct {t} {t};\n  #define {t}_DEFINED\n#endif\n")
            
            for key, sig in sorted(self.func_signatures.items()):
                f.write(f"#ifndef GLOBAL_DEF_{key}\n  #undef {sig.name}\n  #define {sig.name} EH_{key}\n  extern {sig.return_type} EH_{key}({sig.parameters});\n#endif\n")
            
            f.write("#ifdef __cplusplus\n}\n#endif\n#endif\n")

    def run(self):
        print("\n" + "="*60)
        print("Banjo-Kazooie Harmonizer v53.0 EVENT-HORIZON")
        print("="*60)
        ...
        print("="*60)
        print(f"  [PRECHECK] Blocked Duplicate Typedefs: {len(self.predefined_types)}")
        print(f"  [ACHIEVEMENT] New Opaque Types: {len(self.discovered_types)}")
        print(f"  [ACHIEVEMENT] Promoted Symbols: {len(self.func_signatures)}")
        print("="*60 + "\n")

if __name__ == "__main__":
    harmonizer = SourceHarmonizerV53("Android/app/src/main/cpp", "decomp-files")
    harmonizer.run()
