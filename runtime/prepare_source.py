...
class SourceHarmonizerV73:
    ...
    def perform_introspection(self):
        ...
        # Pattern to find enums and defines to avoid creating opaque structs for them
        patterns = [
            re.compile(r'#define\s+([A-Za-z_]\w+)'),
            re.compile(r'typedef\s+.*?\s+(\w+);'),
            re.compile(r'\}\s*(\w+);'),
            re.compile(r'(?:struct|union|enum)\s+(\w+)'),
            re.compile(r'([A-Z][A-Z0-9_]+)\s*=\s*(?:0x)?[0-9A-Fa-f]+') # Captures enum values like AL_MIDI_FX_CTRL_4
        ]
    ...
    def harmonize_logic(self):
        ...
        # Only capture PascalCase or CamelCase as likely custom types
        ...
        for root, _, files in os.walk(self.src_target):
            for f in files:
                ...
                # Type discovery with conflict check
                for t in type_pat.findall(content):
                    if t not in self.conflict_registry and t not in self.immutables:
                        # Extra check: if it's ALL_CAPS, it's likely a constant, not a struct type
                        if not t.isupper():
                ...
                def func_repl(m):
                    ...
                    if name in ('main', 'Entry') or name in self.conflict_registry: return m.group(0)
    ...
    def generate_artifacts(self):
        ...
        with open(header_path, 'w') as f:
            ...
            # Opaque types (Filtered for SDK safety)
