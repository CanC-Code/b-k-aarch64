...
void thread5_sendTaskToQueue(OSMesg msg) {
    // Android HLE: thread5 is not actually running (GIL prevents separate thread).
    // Process messages synchronously in the caller (main thread).
...
void thread5_startF3DEXTask(struct ucode_task_data_s *task_data) {
    ...
    if (task_data == NULL) { extern int g_diag_null_task; g_diag_null_task = 1; return; }
...
void thread5_create(void) {
    ...
    // osStartThread(&sThread5); // disabled for synchronous HLE
}
