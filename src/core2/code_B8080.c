...
BKModel *meshList_createModel(BKMeshList *this, BKVertexList *bk_vtx_list) {
    // MTE-safe: use __builtin_memcpy for reads from tagged source memory
    s16 mesh_count;
    BKModel *model;
    BKMesh *src_mesh;
    ...
    model = (BKModel *) malloc(sizeof(BKModel) + (mesh_count * sizeof(BKModelMesh)) + (meshList_getVtxCount(this) * sizeof(BKModelVtxRef)));
    ...
    for (i = 0; i < mesh_count; i++) {
        BKMesh tmp_mesh;
        __builtin_memcpy(&tmp_mesh, src_mesh, sizeof(BKMesh));
        dst_mesh->uid = tmp_mesh.uid;
        dst_mesh->vtx_count = tmp_mesh.vtx_count;
        vtx_ref = (BKModelVtxRef *) dst_mesh->data;
        for (j = 0; j < tmp_mesh.vtx_count; j++) {
            s16 vtx_id;
        ...
        src_mesh = (BKMesh *) &src_mesh->vertices[tmp_mesh.vtx_count];
